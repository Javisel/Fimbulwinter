...
public class FreezingSnow extends FallingBlock {
    ...
    public FreezingSnow() {
        ...
        setRegistryName(Fimbulwinter.MODID,"freezing_snow");
    }
    @Override
    public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
        if (entityIn.isBurning()) {
            ...
            if (!(entityIn instanceof IFrostMob) && !(entityIn instanceof Sentinel))  {

                if (entityIn instanceof LivingEntity) {

                    entityIn.setMotion(entityIn.getMotion().mul(0.2D, 0.2D, 0.2D));
                    ((LivingEntity) entityIn).addPotionEffect(new EffectInstance(EffectRegistration.FROSTBITE,60,2));
